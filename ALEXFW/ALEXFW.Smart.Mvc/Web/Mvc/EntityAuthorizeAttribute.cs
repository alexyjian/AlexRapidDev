            ...
            else
            {
                if (!filterContext.HttpContext.User.Identity.IsAuthenticated)
                    if (filterContext.RouteData.DataTokens["loginUrl"] == null)
                        filterContext.Result = new RedirectResult((filterContext.HttpContext.Request.ApplicationPath == "/" ? "" : filterContext.HttpContext.Request.ApplicationPath) + ALEXFWAuthentication.LoginUrl + "?returnUrl=" + Uri.EscapeDataString(filterContext.RequestContext.HttpContext.Request.Url.PathAndQuery));
                    else
